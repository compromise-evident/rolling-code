...
{	int raw_byte;
	...
	{	std::cout << "\n(Since digit_length = " << digit_length << ", you'll get a rolling-seed file containing"
		...
		for(int a = 0; a < digit_length; a++) {if(in_stream.get(file_byte)) {raw_byte = file_byte & 0xFF; out_stream.put((raw_byte % 10) + 48);}}
		in_stream.close();
		out_stream.close();
	}
	in_stream.close();
